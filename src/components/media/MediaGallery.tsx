...
const FileIcon = () => (
  <svg viewBox="0 0 24 24" aria-hidden="true" className="h-10 w-10 text-gray-500">
    <path
      fill="currentColor"
...
export function MediaGallery() {
  ...
  const revokePreviewUrl = (item: UploadItem) => {
    if (item.previewUrl) {
      URL.revokeObjectURL(item.previewUrl);
    }
  };
    ...
    try {
      for (const item of pending) {
        ...
        try {
          ...
          if (!presignResponse.ok) throw new Error('Presign fallito');
          ...
          if (!finalizeResponse.ok) throw new Error('Finalize fallito');
    ...
    <div className="space-y-6">
      ...
      >
        ...
        >
          ...
          <div>
            <h2 className="text-sm font-semibold text-gray-800">Trascina qui il file</h2>
          ...
          <input
            ...
            type="file"
            multiple
            onChange={(event) => setQueueWithFiles(Array.from(event.target.files || []))}
            className="sr-only"
          />
        </label>
      ...
      {items.length === 0 ? (
        <p className="text-sm text-gray-500">Nessun media disponibile.</p>
      ) : (
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          {items.map((item) => {
            const preview = getPreviewUrl(item);
            return (
              <div key={item._id} className="rounded-xl border border-gray-200 bg-white/90 p-3 shadow-sm">
                <div className="relative aspect-[4/3] w-full overflow-hidden rounded-lg bg-gradient-to-br from-gray-100 via-gray-50 to-gray-100">
                  <span className="absolute left-2 top-2 rounded-full bg-white/90 px-2 py-0.5 text-[10px] font-semibold text-gray-700 shadow">
                    {item.type === 'video' ? 'Video' : item.type === 'image' ? 'Immagine' : 'File'}
                  </span>
                ...
                <div className="mt-2 text-xs text-gray-600 line-clamp-1">
                  {item.original_filename || item._id}
                </div>
                <div className="text-[10px] uppercase tracking-wide text-gray-400">
